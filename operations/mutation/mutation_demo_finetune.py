...
def main():    
    ...
    try:
        ...
        with open(args.input_file, 'r') as f:
            input_smiles = [line.split()[0].strip() for line in f if line.strip()]
        
        if not input_smiles:
            raise ValueError("输入文件为空")
        
        print(f"从输入文件读取了 {len(input_smiles)} 个分子")
        ...
        # 合并种群用于评估
        full_population = list(set(input_smiles + llm_smiles))
        ...
        vars = {
            ...
            'filter_object_dict': {
                'Structure_check': StructureCheckFilter()
            },
        ...
        print("*****************************开始变异*****************************")
        mutation_results = []
        max_tries = args.mutation_attempts * 10  # 设置最大尝试次数
        
        with tqdm(total=args.mutation_attempts, desc="Performing mutations") as pbar:
            tries = 0
            while len(mutation_results) < args.mutation_attempts and tries < max_tries:
                tries += 1          
                
                # 随机选择一个父代分子
                parent = random.choice(input_smiles)
                ...
                for attempt in range(args.max_mutations):
                    ...
                    for smi in result:
                        try:
                            ...
                            if all([filter_obj.run_filter(mol) for filter_obj in vars['filter_object_dict'].values()]):
                    ...
                    if valid_results:
                        ...
                        if chosen_smi not in input_smiles and chosen_smi not in mutation_results:
        ...
        # 如果变异产生的分子不足，使用随机选择的输入分子补充
        if len(mutation_results) < args.mutation_attempts:
            needed = args.mutation_attempts - len(mutation_results)
            print(f"变异产生的分子不足，需要从输入分子随机选择 {needed} 个分子补充")
            
            available_mols = [mol for mol in input_smiles if mol not in mutation_results]
            
            if available_mols:
                selected_mols = random.sample(available_mols, min(needed, len(available_mols)))
                mutation_results.extend(selected_mols)
        ...
        # 评估结果
        eval_results = evaluate_population(mutation_results, div_eval, nov_eval, qed_eval, sa_eval, full_population)
        print(f"变异结果评估:")
