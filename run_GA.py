...
PARSER.add_argument(
    ...
    metavar="mgl_python",
    required=False,
    default="/data1/tgy/GA_llm/mgltools_x86_64Linux2_1.5.6/bin/pythonsh",
    help="/data1/tgy/GA_llm/mgltools_x86_64Linux2_1.5.6/bin/pythonsh",
)
PARSER.add_argument(
    "--prepare_ligand4.py",
    metavar="prepare_ligand4.py",
    required=False,
    default="/data1/tgy/GA_llm/mgltools_x86_64Linux2_1.5.6/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py",
    help="/data1/tgy/GA_llm/mgltools_x86_64Linux2_1.5.6/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_ligand4.py",
)
PARSER.add_argument(
...
PARSER.add_argument(
    ...
    default="/data1/tgy/GA_llm/autogrow/docking/docking_executables/vina/autodock_vina_1_1_2_linux_x86/bin/vina",
...
相当于"空运行"
    使用场景：
MPI并行环境的第一阶段(准备阶段）
防止多进程同时首次导入模块时的文件访问冲突
提前编译模块以减少后续实际计算时的延迟
确保所有计算节点加载相同版本的代码
"""
PARSER.add_argument(
    "--cache_prerun",   #为了解决MPI并行计算中的一个特殊需求设计的；取值为：false时：正常执行模式
    "-c",
    action="store_true",
...
PARSER.add_argument(
    "--random_seed",
    type=int,
    default=42,  # 设置一个默认种子值
    help="Random seed for reproducibility. Using the same seed will produce identical results.",
)

args_dict = vars(PARSER.parse_args())

# copying args_dict so we can delete out of while iterating through the original args_dict
...
if args_dict["cache_prerun"] is False:
    ...
    # 设置随机种子，确保结果可复现
    random.seed(vars["random_seed"])
    print(f"Set Python random seed to: {vars['random_seed']}")

    # numpy
    try:
        import numpy as np
        np.random.seed(vars["random_seed"])
        print(f"Set NumPy random seed to: {vars['random_seed']}")
    except ImportError:
        pass
    ...
    try:
        ...
        RDRandom.seed(vars["random_seed"])
        print(f"Set RDKit random seed to: {vars['random_seed']}")
    except ImportError:
        pass
    ...
    # errors
    #调用方法main_execute
    import autogrow.GA_main_execute as GAMainExecute

    if "random_seed" not in vars:
        vars["random_seed"] = args_dict["random_seed"]  # 确保将种子值添加到vars字典中

    GAMainExecute.main_execute(vars)

    # Print completion message

    printout = "\nGA run started at:   {}\nGA ".format(start_time)
