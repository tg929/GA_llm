...
处理流程说明:
- 优化了种群大小控制，每代产生固定数量的新个体
- 使用种子分子作为唯一来源进行交叉和变异，防止种群规模暴增
- 引入了种子选择机制，提高进化效率
...
def run_crossover_with_seeds(source_file, output_file, seed_list, num_crossovers, gen_num, logger):
    """
    运行分子交叉，使用选定的种子分子
    ...
    crossover_script = os.path.join(PROJECT_ROOT, "operations/crossover/crossover_demo_GA.py")
    cmd = [
        "python", crossover_script,
        "--source_compound_file", seed_file,
...
def run_mutation_with_seeds(input_file, output_file, seed_list, num_mutations, gen_num, logger):
    """
    运行分子变异，使用选定的种子分子
    ...
    mutation_script = os.path.join(PROJECT_ROOT, "operations/mutation/mutation_demo_GA.py")
    cmd = [
...
def run_evolution(generation_num, args, logger):
    ...
    if not crossover_molecules and not mutation_molecules:
        ...
        # 合并交叉和变异产生的分子
        merged_molecules = crossover_molecules + mutation_molecules
        
        # 控制合并后的种群大小，确保不会超过设定的交叉和变异数量之和
        max_merged_size = num_crossovers + num_mutations
        if len(merged_molecules) > max_merged_size:
            logger.info(f"合并后分子数量({len(merged_molecules)})超过预期({max_merged_size})，随机抽样限制大小")
            merged_molecules = random.sample(merged_molecules, max_merged_size)
        
        with open(merged_output, 'w') as f:
            for smile in merged_molecules:
                f.write(f"{smile}\n")
    
    merged_molecule_count = len(get_molecules_from_file(merged_output))
    logger.info(f"合并交叉和变异结果: 交叉 {len(crossover_molecules)} 个, 变异 {len(mutation_molecules)} 个, 合并后 {merged_molecule_count} 个")
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='GA.py - 纯遗传算法分子优化迭代')
    
    # 基本参数
    parser.add_argument('--generations', type=int, default=5, 
                        help='进化总代数 (generation_0到generation_5)')
    parser.add_argument('--output_dir', type=str, default='/data1/tgy/GA_llm/output_ga/',
                        help='输出目录')
    ...
    if args.number_of_processors != 1 and args.multithread_mode == "serial":
        print(f"检测到可能使用多核({args.number_of_processors})但模式为serial,自动切换为multithreading模式")
